...
void AleOptimizer::inferMLReconciliations() {
  ...
  for (unsigned int i = 0; i < localFamilies.size(); ++i) {
    ...
    auto geneTreePath = FileSystem::joinPaths(
        allRecDir, localFamilies[i].name + std::string("_sample_ml.newick"));
    auto geneTreeAlePath = FileSystem::joinPaths(
        allRecDir, localFamilies[i].name + std::string("_sample_ml.alerec"));
    auto geneTreeXMLPath = FileSystem::joinPaths(
        allRecDir, localFamilies[i].name + std::string("_sample_ml.xml"));
    auto eventCountsFile = FileSystem::joinPaths(
        allRecDir, localFamilies[i].name + std::string("_eventCounts_ml.txt"));
    ...
    auto transferFile = FileSystem::joinPaths(
        allRecDir, localFamilies[i].name + std::string("_transfers_ml.txt"));
