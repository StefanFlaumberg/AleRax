...
inline
void MultiModelInterface::mapGenesToSpecies()
{
...
inline
void MultiModelInterface::updateSpeciesToPrunedNode()
{
 ...
 */
template<class REAL>
class MultiModel: public MultiModelInterface {
public:
  MultiModel(PLLRootedTree &speciesTree,
      const GeneSpeciesMapping &geneSpeciesMapping,
      const RecModelInfo &info,
      const std::string &ccpFile):
    MultiModelInterface(speciesTree,
        geneSpeciesMapping,
        info,
        ccpFile)
  {}
...
private:
   ...
   */
  bool computeScenario(Scenario &scenario,
      bool stochastic);

  /**
   *  Recursively sample a reconciled gene tree
   */
  bool backtrace(CID cid,
      corax_rnode_t *speciesNode,
...
template <class REAL>
double MultiModel<REAL>::computeLogLikelihood()
{
  if (this->_ccp.skip()) {
    return 0.0;
  }
  ...
  res = std::accumulate(categoryLikelihoods.begin(), categoryLikelihoods.end(), REAL());
...
template <class REAL>
bool MultiModel<REAL>::sampleReconciliations(unsigned int samples,
    std::vector< std::shared_ptr<Scenario> > &scenarios)
{
  if (this->_memorySavings) {
    // initialize CLVs, as they have not been created with the model
  ...
  for (unsigned int i = 0; i < samples; ++i) {
    ...
    auto ok = computeScenario(*scenarios.back(), true);
    if (!ok) {
      deallocateMemory();
      return false;
    }
...
template <class REAL>
void MultiModel<REAL>::updateCLVs()
{
    ...
    unsigned int &category)
{
  ...
  auto toSample = totalLikelihood * Random::getProba();
...
template <class REAL>
bool MultiModel<REAL>::backtrace(CID cid,
    corax_rnode_t *speciesNode,
    corax_unode_t *geneNode,
    unsigned int category,
    Scenario &scenario,
