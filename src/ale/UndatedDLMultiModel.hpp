  ...
  _originationStrategy(info.originationStrategy)
{
  ...
  _dlRates.resize(this->_info.modelFreeParameters(), std::vector<double>(N, 0.2));
...
template <class REAL>
void UndatedDLMultiModel<REAL>::setAlpha(double alpha)
{
 ...
 */
template<class REAL>
void UndatedDLMultiModel<REAL>::allocateMemory()
{
 ...
 */
template<class REAL>
void UndatedDLMultiModel<REAL>::deallocateMemory()
{
 ...
 */
template <class REAL>
void UndatedDLMultiModel<REAL>::updateCLV(CID cid)
{
  auto &uq = _dlclvs[cid];
 ...
 */
template <class REAL>
REAL UndatedDLMultiModel<REAL>::getRootCladeLikelihood(corax_rnode_t *speciesNode,
    unsigned int category)
{
    ...
    bool stochastic)
{
  ...
  if (!this->_info.noVirtualEvents) {
    // - DL event
    temp = proba / (1 - (_uE[ec] * _PD[ec] * 2.0));
    scale(temp);
    proba = (stochastic) ? temp : REAL(); // can never be the ML event
    if (recCell && proba >= maxProba) {
